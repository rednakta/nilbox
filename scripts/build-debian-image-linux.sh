...
# build-debian-image-linux.sh — Debian minimal VM image for Linux (auto-detect arch)
# Fully non-interactive; uses Docker privileged container for debootstrap.
# Outputs: RAW disk image + kernel + initrd for QEMU + KVM.
#
# Usage:
#   ./build-debian-image-linux.sh [--dbus]
#
# Options:
...
case "${HOST_ARCH}" in
    aarch64)
        ...
        QEMU_MACHINE="-machine virt,accel=kvm,gic-version=3 -cpu host"
    ...
    x86_64)
        ...
        QEMU_MACHINE="-machine pc,accel=kvm -cpu host"
...
IMAGE_NAME="debian-${DEBIAN_VERSION}-nilbox-linux-${GUEST_ARCH}.img"
...
if [[ -e /dev/kvm ]]; then
    if [[ -r /dev/kvm && -w /dev/kvm ]]; then
        info "KVM: OK (/dev/kvm accessible)"
    else
        warn "KVM: /dev/kvm exists but not accessible. Add user to kvm group: sudo usermod -aG kvm \$USER"
    fi
else
    warn "KVM: /dev/kvm not found. VM will run without hardware acceleration (slow)."
    warn "  Enable KVM: sudo modprobe kvm kvm_intel (or kvm_amd)"
fi
...
if ! command -v file &>/dev/null; then
    die "'file' command not found. Install: sudo apt install file"
fi
info "file: OK"
...
echo "[container] Starting Debian ${DEBIAN_VERSION} ${GUEST_ARCH} image build (Linux target)"
...
# Linux uses vhost-vsock-pci: vsock modules are required (unlike Windows)
KEEP_MODS="vsock vmw_vsock_virtio_transport_common vmw_vsock_virtio_transport virtio_console fuse virtio_rng qemu_fw_cfg ip_tables iptable_nat iptable_filter nf_nat nf_conntrack nf_defrag_ipv4 nf_defrag_ipv6 x_tables xt_REDIRECT libcrc32c crc32c_generic dummy"
...
echo
echo "─────────────────────────────────────────────────────────────"
echo " QEMU + KVM direct boot (Linux ${GUEST_ARCH})"
echo "─────────────────────────────────────────────────────────────"
...
echo
echo "─────────────────────────────────────────────────────────────"
echo " nilbox JSON config (QEMU + KVM)"
echo "─────────────────────────────────────────────────────────────"
cat << JSONCONFIG_EOF
{
  "cmd": "start",
...
echo
echo "─────────────────────────────────────────────────────────────"
echo " KVM prerequisite (if /dev/kvm is missing)"
echo "─────────────────────────────────────────────────────────────"
echo "  sudo modprobe kvm kvm_intel   # Intel CPU"
echo "  sudo modprobe kvm kvm_amd     # AMD CPU"
echo "  sudo usermod -aG kvm \$USER    # Allow non-root access"
echo "  sudo modprobe vhost_vsock      # VSOCK host support"
echo
echo "Done!"
