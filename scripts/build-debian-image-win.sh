...
# build-debian-image-win.sh — Debian minimal VM image for Windows (x86_64)
# Fully non-interactive; uses Rancher Desktop (nerdctl) privileged container for debootstrap.
# Outputs: RAW disk image + kernel + initrd for QEMU + WHPX.
#
# Usage:
#   ./build-debian-image-win.sh [--dbus]
#
# Options:
...
# ─── Windows path helper ────────────────────────────────────────────────────
# Docker Desktop for Windows requires native paths (C:\...), not MSYS /c/... paths.
to_winpath() {
...
# ─── Architecture (Windows = x86_64 only) ──────────────────────────────────
...
info_arch() { echo "[INFO]  Architecture: x86_64 → guest ${GUEST_ARCH}, target ${RUST_TARGET}"; }
...
IMAGE_NAME="debian-${DEBIAN_VERSION}-nilbox-win-${GUEST_ARCH}.img"

# ─── Container runtime detection (nerdctl / docker) ────────────────────────
...
if ! docker info &>/dev/null 2>&1; then
    die "docker daemon is not running. Start Rancher Desktop first."
fi
info "docker: OK"
info_arch
...
if command -v qemu-img &>/dev/null; then
    ...
    warn "qemu-img not found (optional — only needed for image conversion)"
fi
...
if [[ -f "${VM_AGENT_BIN}" && -f "${NILBOX_INSTALL_BIN}" && -f "${MCP_STDIO_PROXY_BIN}" ]]; then
    ...
    MSYS_NO_PATHCONV=1 docker build \
        ...
        -f "$(to_winpath "${VM_AGENT_DIR}/Dockerfile")" \
        "$(to_winpath "${VM_AGENT_DIR}")"
    ...
    BUILDER_CONTAINER="nilbox-rust-builder-run"
    ...
    if [[ -n "${CONTAINER_IMAGE_ID}" && "${CONTAINER_IMAGE_ID}" == "${NEW_IMAGE_ID}" ]]; then
        ...
        MSYS_NO_PATHCONV=1 docker start -a "${BUILDER_CONTAINER}"
    else
        ...
        MSYS_NO_PATHCONV=1 docker run \
            ...
            -v "$(to_winpath "${NILBOX_ROOT}"):/workspace" \
            nilbox-rust-builder \
            sh /workspace/vm-agent/build.win.sh
    fi
...
# ─── Step 4: Rancher/nerdctl privileged build (debootstrap) ──────────────────

step "Step 4: Running debootstrap in privileged container (docker)"

# Write the container script into OUTPUT_DIR (mounted as /output).
...
# Config comes from -e environment variables passed by container run:
#   IMAGE_FILE, DEBIAN_VERSION, ROOT_PASSWORD, VM_HOSTNAME,
#   GUEST_ARCH, KERNEL_PKG, CONSOLE_SERIAL, GOTASK_ARCH

echo "[container] Starting Debian ${DEBIAN_VERSION} ${GUEST_ARCH} image build (Windows target)"
...
# Blacklist vsock modules — Windows QEMU has no vhost-vsock-pci backend.
# Without this, /dev/vsock exists and vm-agent picks vsock over virtio-serial.
cat > /mnt/debian/etc/modprobe.d/no-vsock.conf << 'EOF'
...
KEEP_MODS="virtio_console fuse virtio_rng qemu_fw_cfg ip_tables iptable_nat iptable_filter nf_nat nf_conntrack nf_defrag_ipv4 nf_defrag_ipv6 x_tables xt_REDIRECT libcrc32c crc32c_generic dummy"
...
MSYS_NO_PATHCONV=1 docker run --rm \
    ...
    -v "$(to_winpath "${OUTPUT_DIR}"):/images" \
    -v "$(to_winpath "${OUTPUT_DIR}"):/output" \
    -v "$(to_winpath "${VM_AGENT_BIN}"):/vm-agent-bin:ro" \
    -v "$(to_winpath "${NILBOX_INSTALL_BIN}"):/nilbox-install-bin:ro" \
    -v "$(to_winpath "${MCP_STDIO_PROXY_BIN}"):/mcp-stdio-proxy-bin:ro" \
...
if echo "${KERNEL_INFO}" | grep -q "gzip compressed\|GZIP"; then
    info "Kernel is gzip-compressed — extracting raw ELF..."
    ...
    info "Kernel format not recognized as gzip or ELF — QEMU may handle it natively"
fi
...
echo
echo "─────────────────────────────────────────────────────────────"
echo " QEMU + WHPX direct boot (Windows x86_64)"
echo " [prerequisite] Enable Windows Hypervisor Platform (admin PowerShell):"
echo "   Get-WindowsOptionalFeature -Online -FeatureName HypervisorPlatform"
echo "   Enable-WindowsOptionalFeature -Online -FeatureName HypervisorPlatform"
echo " Tested: QEMU 10.2.0, QEMU 9.2 — run from nilbox project root"
echo "─────────────────────────────────────────────────────────────"
cat << QEMU_EOF
"C:\Program Files\qemu\qemu-system-x86_64.exe" \\
  ...
  -drive file=debian-vm/${IMAGE_NAME},if=virtio,format=raw \\
  -nic user,model=virtio-net-pci
QEMU_EOF

echo
echo "─────────────────────────────────────────────────────────────"
echo " nilbox JSON config (QEMU + WHPX)"
echo "─────────────────────────────────────────────────────────────"
cat << JSONCONFIG_EOF
{
  "cmd": "start",
